...
static char    hmi_plot_pos_readout_cache[24];
static char    hmi_plot_vel_readout_cache[24];
static uint8_t hmi_plot_readout_valid = 0U;
static uint8_t hmi_plot_readout_sample_valid = 0U;
static float   hmi_plot_readout_last_pos_deg = 0.0f;
static float   hmi_plot_readout_last_vel_rpm = 0.0f;
static uint32_t hmi_plot_readout_last_ms = 0U;
static uint32_t hmi_last_plot_readout_refresh_ms = 0U;
...
static float HMI_WrapDeltaDeg180(float delta_deg)
{
    delta_deg = fmodf(delta_deg, 360.0f);
    if (delta_deg > 180.0f)
    {
        delta_deg -= 360.0f;
    }
...
static float HMI_AbsF(float value)
{
...
static void HMI_ResetPlotReadout(void)
{
    hmi_plot_pos_readout_cache[0] = '\0';
    hmi_plot_vel_readout_cache[0] = '\0';
    hmi_plot_readout_valid = 0U;
    hmi_plot_readout_sample_valid = 0U;
    hmi_plot_readout_last_pos_deg = HMI_WrapDeg180(encoder_measured_position1);
    hmi_plot_readout_last_vel_rpm = measured_velocity1;
    hmi_plot_readout_last_ms = HAL_GetTick();
    hmi_last_plot_readout_refresh_ms = 0U;
}

static void HMI_DrawPlotReadoutField(uint16_t x, const char *text)
{
    ILI9341_fillRect(x,
                     HMI_PLOT_READOUT_Y,
                     (uint16_t)(HMI_PLOT_READOUT_CHARS * HMI_CHAR_W),
                     HMI_PLOT_READOUT_H,
                     HMI_BG);
    HMI_PrintFixedWidth(x,
                        HMI_PLOT_READOUT_Y,
                        text,
                        HMI_PLOT_READOUT_CHARS,
                        HMI_FG,
                        HMI_BG);
}

static void HMI_UpdatePlotReadout(uint8_t force)
{
    char pos_text[12];
    char vel_text[12];
    char pos_line[24];
    ...
    if ((force == 0U) &&
        ((now - hmi_last_plot_readout_refresh_ms) < HMI_PLOT_READOUT_REFRESH_MS))
    {
        return;
    }
    hmi_last_plot_readout_refresh_ms = now;

    float pos_deg = HMI_WrapDeg180(encoder_measured_position1);
    float vel_rpm = measured_velocity1;
    uint8_t pos_stable = 1U;
    uint8_t vel_stable = 1U;

    if (hmi_plot_readout_sample_valid != 0U)
    {
        uint32_t elapsed_ms = now - hmi_plot_readout_last_ms;
        if (elapsed_ms > 0U)
        {
            float dt = ((float)elapsed_ms) * 0.001f;
            float pos_rate_dps = HMI_AbsF(HMI_WrapDeltaDeg180(pos_deg - hmi_plot_readout_last_pos_deg)) / dt;
            float vel_rate_rpmps = HMI_AbsF(vel_rpm - hmi_plot_readout_last_vel_rpm) / dt;

            if (pos_rate_dps > HMI_PLOT_POS_STABLE_DPS)
            {
                pos_stable = 0U;
            }
            if (vel_rate_rpmps > HMI_PLOT_VEL_STABLE_RPMPS)
            {
                vel_stable = 0U;
            }
        }
    ...
    hmi_plot_readout_last_pos_deg = pos_deg;
    hmi_plot_readout_last_vel_rpm = vel_rpm;
    hmi_plot_readout_last_ms = now;
    hmi_plot_readout_sample_valid = 1U;

    if (pos_stable != 0U)
    {
        HMI_FloatToStr(pos_text, sizeof(pos_text), pos_deg, 1U);
    }
    else
    {
        (void)snprintf(pos_text, sizeof(pos_text), "xxxx");
    }

    if (vel_stable != 0U)
    {
        HMI_FloatToStr(vel_text, sizeof(vel_text), vel_rpm, 1U);
    }
    else
    {
        (void)snprintf(vel_text, sizeof(vel_text), "xxxx");
    }

    (void)snprintf(pos_line, sizeof(pos_line), "POS=%s deg", pos_text);
    (void)snprintf(vel_line, sizeof(vel_line), "VEL=%s rpm", vel_text);

    if ((hmi_plot_readout_valid == 0U) ||
        (strcmp(hmi_plot_pos_readout_cache, pos_line) != 0))
    {
        HMI_DrawPlotReadoutField(HMI_PLOT_POS_READOUT_X, pos_line);
        (void)snprintf(hmi_plot_pos_readout_cache,
                       sizeof(hmi_plot_pos_readout_cache),
                       "%s",
                       pos_line);
    }

    if ((hmi_plot_readout_valid == 0U) ||
        (strcmp(hmi_plot_vel_readout_cache, vel_line) != 0))
    {
        HMI_DrawPlotReadoutField(HMI_PLOT_VEL_READOUT_X, vel_line);
        (void)snprintf(hmi_plot_vel_readout_cache,
                       sizeof(hmi_plot_vel_readout_cache),
                       "%s",
                       vel_line);
    }

    hmi_plot_readout_valid = 1U;
}
...
static void HMI_DrawPlotBase(const char *title)
{
    ...
    ILI9341_fillRect(HMI_PLOT_READOUT_X, HMI_PLOT_READOUT_Y,
                     HMI_PLOT_READOUT_W, HMI_PLOT_READOUT_H, HMI_BG);
