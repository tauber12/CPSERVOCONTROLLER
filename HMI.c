...
#define FAULT_BOX_X                    (MODE_SLOT_2_X + MODE_BOX_W + MODE_GAP)
#define FAULT_BOX_W                    64U
#define FAULT_FLASH_MS                 250U
...
#define HMI_FLOAT_MAX_KP               5.0f
#define HMI_FLOAT_MIN_KI               0.0f
#define HMI_FLOAT_MAX_KI               5.0f
#define HMI_FLOAT_STEP_GAIN            0.025f
...
#define FAULT_SCREEN_TITLE_Y           14U
#define FAULT_SCREEN_LIST_X            30U
#define FAULT_SCREEN_LIST_Y            64U
#define FAULT_SCREEN_RESET_X           90U
#define FAULT_SCREEN_RESET_Y           198U
#define FAULT_SCREEN_RESET_W           140U
#define FAULT_SCREEN_RESET_H           28U
...
static void HMI_FaultButtonRender(UiItem_t *self);
static void HMI_FaultButtonPress(UiItem_t *self);
...
static UiItem_t hmi_items[HMI_ITEM_COUNT] =
{
    ...
    [HMI_INDEX_FAULT]    = { UI_ITEM_BUTTON, "FAULT",    FAULT_BOX_X,   MODE_BOX_Y, FAULT_BOX_W, MODE_BOX_H, 1U, 0U, 0U, 0U, COLOR_RED,   NULL, NULL, HMI_FaultButtonRender, HMI_FaultButtonPress, NULL },
    ...
    [HMI_INDEX_INPUT_SOURCE] = { UI_ITEM_INPUT_SELECT, "SRC", INPUT_FIELD_X, FIELD_Y(VALUE_ROW_0), INPUT_FIELD_W, FIELD_H, 1U, 0U, 0U, 9U, COLOR_ORANGE, NULL, HMI_FormatInputSource, HMI_InputSelectRender, HMI_InputSelectPress, HMI_InputSelectEncoder },
    [HMI_INDEX_INPUT_FREQ]   = { UI_ITEM_FLOAT_ENTRY, "Fq", INPUT_FIELD_X, FIELD_Y(VALUE_ROW_1), INPUT_FIELD_W, FIELD_H, 1U, 0U, 0U, 9U, COLOR_ORANGE, &hmi_input_freq, HMI_FormatFloatEntry, HMI_FloatEntryRender, HMI_FloatEntryPress, HMI_FloatEntryEncoder },
    [HMI_INDEX_INPUT_AMP]    = { UI_ITEM_FLOAT_ENTRY, "Amp", INPUT_FIELD_X, FIELD_Y(VALUE_ROW_2), INPUT_FIELD_W, FIELD_H, 1U, 0U, 0U, 9U, COLOR_ORANGE, &hmi_input_amp, HMI_FormatFloatEntry, HMI_FloatEntryRender, HMI_FloatEntryPress, HMI_FloatEntryEncoder },
    ...
    [HMI_INDEX_POS_FS]       = { UI_ITEM_FLOAT_ENTRY, "Fs",  POS_FIELD_X, FIELD_Y(VALUE_ROW_2), POS_FIELD_W, FIELD_H, 1U, 0U, 0U, 10U, COLOR_RED, &hmi_pos_fs, HMI_FormatFloatEntry, HMI_FloatEntryRender, HMI_FloatEntryPress, HMI_FloatEntryEncoder },
    ...
    [HMI_INDEX_VEL_FS]       = { UI_ITEM_FLOAT_ENTRY, "Fs",  VEL_FIELD_X, FIELD_Y(VALUE_ROW_2), VEL_FIELD_W, FIELD_H, 1U, 0U, 0U, 10U, COLOR_BLUE, &hmi_vel_fs, HMI_FormatFloatEntry, HMI_FloatEntryRender, HMI_FloatEntryPress, HMI_FloatEntryEncoder }
};
...
static uint8_t hmi_fault_flash_on = 0U;
static uint8_t hmi_fault_present_last = 0U;
static uint32_t hmi_last_fault_flash_ms = 0U;
static uint8_t hmi_fault_reset_failed = 0U;
...
#define HMI_RENDER_CACHE_TEXT_LEN       32U

static char    hmi_render_cache[HMI_ITEM_COUNT][HMI_RENDER_CACHE_TEXT_LEN];
static uint8_t hmi_render_cache_valid[HMI_ITEM_COUNT];
static uint8_t hmi_render_cache_selected[HMI_ITEM_COUNT];
static uint8_t hmi_render_cache_on[HMI_ITEM_COUNT];
static uint8_t hmi_render_cache_editing[HMI_ITEM_COUNT];
...
static uint8_t HMI_ItemEditingState(const UiItem_t *item)
{
    return ((item != NULL) &&
            (item->is_selected != 0U) &&
            (hmi_editing != 0U) &&
            ((item->type == UI_ITEM_FLOAT_ENTRY) ||
             (item->type == UI_ITEM_INPUT_SELECT))) ? 1U : 0U;
}

static void HMI_BuildItemText(uint8_t index, char *dst, size_t dst_len)
{
    ...
    UiItem_t *item = &hmi_items[index];
    if (item->format != NULL)
    {
        item->format(item, dst, dst_len);
    }
    else if (item->label != NULL)
    {
        (void)snprintf(dst, dst_len, "%s", item->label);
    }
}

static void HMI_InvalidateItem(uint8_t index)
{
    if (index < HMI_ITEM_COUNT)
    {
        hmi_render_cache_valid[index] = 0U;
        hmi_render_cache[index][0] = '\0';
    }
}

static void HMI_InvalidateAllItems(void)
{
    for (uint8_t i = 0U; i < HMI_ITEM_COUNT; i++)
    {
        HMI_InvalidateItem(i);
    }
}

static void HMI_UpdateItemCache(uint8_t index, const char *text)
{
    if (index >= HMI_ITEM_COUNT)
    {
        return;
    ...
    {
        (void)snprintf(hmi_render_cache[index],
                       sizeof(hmi_render_cache[index]),
                       "%s",
                       text);
    }

    hmi_render_cache_selected[index] = item->is_selected;
    hmi_render_cache_on[index] = item->is_on;
    hmi_render_cache_editing[index] = HMI_ItemEditingState(item);
    hmi_render_cache_valid[index] = 1U;
}

static uint8_t HMI_ItemCacheMatches(uint8_t index, const char *text)
{
    if ((index >= HMI_ITEM_COUNT) ||
        (hmi_render_cache_valid[index] == 0U) ||
        (text == NULL))
    ...
    UiItem_t *item = &hmi_items[index];
    if (hmi_render_cache_selected[index] != item->is_selected)
    {
        return 0U;
    }
    if (hmi_render_cache_on[index] != item->is_on)
    {
        return 0U;
    }
    if (hmi_render_cache_editing[index] != HMI_ItemEditingState(item))
    {
        return 0U;
    }

    return (strcmp(hmi_render_cache[index], text) == 0) ? 1U : 0U;
}

static void HMI_ClearItemArea(uint8_t index)
{
    if (index >= HMI_ITEM_COUNT)
    {
        return;
    ...
    UiItem_t *item = &hmi_items[index];
    ILI9341_fillRect(item->x, item->y, item->w, item->h, HMI_BG);
    HMI_InvalidateItem(index);
}
...
static void HMI_RenderItemEx(uint8_t index, uint8_t force)
{
    char text[HMI_RENDER_CACHE_TEXT_LEN];
    ...
    HMI_BuildItemText(index, text, sizeof(text));
    if ((force == 0U) && (HMI_ItemCacheMatches(index, text) != 0U))
    {
        return;
    }

    hmi_items[index].render(&hmi_items[index]);
    HMI_UpdateItemCache(index, text);
}

static void HMI_RenderItem(uint8_t index)
{
    HMI_RenderItemEx(index, 1U);
}

static void HMI_RenderItemIfChanged(uint8_t index)
{
    HMI_RenderItemEx(index, 0U);
}
...
static void HMI_RenderTopItemsIfChanged(void)
{
    for (uint8_t i = HMI_INDEX_VELOCITY; i <= HMI_INDEX_PRESET2; i++)
    {
        HMI_RenderItemIfChanged(i);
    }
}
...
static void HMI_RenderDiagramItemsIfChanged(void)
{
    for (uint8_t i = HMI_INDEX_INPUT_SOURCE; i < HMI_ITEM_COUNT; i++)
    {
        HMI_RenderItemIfChanged(i);
...
static uint8_t HMI_SyncButtonStates(void)
{
    ...
    uint8_t fault_present = Control_HasAnyFault();
    uint32_t now = HAL_GetTick();

    if (fault_present != hmi_fault_present_last)
    {
        hmi_fault_present_last = fault_present;
        hmi_fault_flash_on = fault_present;
        hmi_last_fault_flash_ms = now;
        changed = 1U;
    }
    else if (fault_present != 0U)
    {
        if ((now - hmi_last_fault_flash_ms) >= FAULT_FLASH_MS)
        {
            hmi_last_fault_flash_ms = now;
            hmi_fault_flash_on = hmi_fault_flash_on ? 0U : 1U;
            changed = 1U;
        }
    }
    else
    {
        hmi_fault_flash_on = 0U;
        hmi_last_fault_flash_ms = now;
    }

    uint8_t fault_on = ((fault_present != 0U) && (hmi_fault_flash_on != 0U)) ? 1U : 0U;
    ...
    if (hmi_items[HMI_INDEX_FAULT].is_on != fault_on)
    {
        hmi_items[HMI_INDEX_FAULT].is_on = fault_on;
        changed = 1U;
    }
    return changed;
}
...
{
    uint8_t fault_present = Control_HasAnyFault();
    uint16_t fill = ((fault_present != 0U) && (self->is_on != 0U)) ? COLOR_RED : HMI_BG;
    uint16_t fg = ((fault_present != 0U) && (self->is_on != 0U)) ? COLOR_WHITE :
                  ((fault_present != 0U) ? COLOR_RED : HMI_FG);
    uint16_t border = self->is_selected ? HMI_SELECTED :
                      ((fault_present != 0U) ? COLOR_RED : HMI_DIM);
...
static void HMI_OpenFaultScreen(void);

static void HMI_RequestStartToggle(void)
{
    hmi_editing = 0U;

    if (Control_HasAnyFault() != 0U)
    {
        return;
    }
...
static void HMI_HandleInputSourceChanged(ControlInputSource_t old_source)
{
    uint8_t old_had_wave_entries = (old_source == CONTROL_INPUT_USER) ? 0U : 1U;
    uint8_t new_has_wave_entries = (control_input_source == CONTROL_INPUT_USER) ? 0U : 1U;

    HMI_RenderItem(HMI_INDEX_INPUT_SOURCE);

    if (old_had_wave_entries != new_has_wave_entries)
    {
        if (new_has_wave_entries != 0U)
        {
            HMI_RenderItem(HMI_INDEX_INPUT_FREQ);
            HMI_RenderItem(HMI_INDEX_INPUT_AMP);
        }
        else
        {
            HMI_ClearItemArea(HMI_INDEX_INPUT_FREQ);
            HMI_ClearItemArea(HMI_INDEX_INPUT_AMP);
        }
    }
    else if (new_has_wave_entries != 0U)
    {
        HMI_RenderItemIfChanged(HMI_INDEX_INPUT_FREQ);
        HMI_RenderItemIfChanged(HMI_INDEX_INPUT_AMP);
    }
}
...
static void HMI_InputSelectEncoder(UiItem_t *self, int32_t steps)
{
    ...
    {
        HMI_HandleInputSourceChanged(old_source);
    }
}
...
static void HMI_HandleEncoderSteps(int32_t steps)
{
    ...
    if ((hmi_editing != 0U) && (selected->on_encoder != NULL))
    {
        ...
        HMI_RenderItemIfChanged(hmi_selected_index);
    }
    else
    {
...
static const HmiYAxisTick_t hmi_position_y_ticks[] =
{
    { -180.0f, "-180" },
    { -135.0f, NULL },
    {  -90.0f, "-90" },
...
static const HmiYAxisTick_t hmi_velocity_y_ticks[] =
{
    { -500.0f, "-500" },
    { -375.0f, NULL },
    { -250.0f, "-250" },
...
static const HmiYAxisTick_t *HMI_GetYAxisTicks(uint8_t *count)
{
    if (desired_state == STATE_VELOCITY_CONTROL)
    {
        if (count != NULL)
        {
            *count = (uint8_t)(sizeof(hmi_velocity_y_ticks) / sizeof(hmi_velocity_y_ticks[0]));
        }
        return hmi_velocity_y_ticks;
    }

    if (count != NULL)
    {
        *count = (uint8_t)(sizeof(hmi_position_y_ticks) / sizeof(hmi_position_y_ticks[0]));
    }
    return hmi_position_y_ticks;
}

static void HMI_PrintYAxisLabel(const char *label, int16_t y)
{
    if (label == NULL)
    {
        return;
    ...
    int16_t x = (int16_t)HMI_PLOT_X - 4 - (int16_t)HMI_TextWidth(label);
    if (x < 0)
    {
        x = 0;
    }

    int16_t y_text = (int16_t)(y - 3);
    if (y_text < 0)
    {
        y_text = 0;
    }
    ...
    ILI9341_printString((uint16_t)x, (uint16_t)y_text, label, HMI_DIM, HMI_BG, HMI_TEXT_SCALE);
}

static void HMI_DrawYAxisTickMark(int16_t y, uint8_t major)
{
    uint16_t tick_len = (major != 0U) ? 3U : 2U;
    HMI_DrawLine((int16_t)(HMI_PLOT_X - tick_len),
                 y,
                 (int16_t)HMI_PLOT_X,
                 y,
                 HMI_DIM);
...
static void HMI_DrawYMajorGridLine(int16_t y)
{
    HMI_DrawLine((int16_t)(HMI_PLOT_X + 1U),
                 y,
                 (int16_t)(HMI_PLOT_X + HMI_PLOT_W - 2U),
                 y,
                 HMI_DIM);
}

static void HMI_DrawYAxisScale(float min_value, float max_value)
{
    uint8_t tick_count = 0U;
    const HmiYAxisTick_t *ticks = HMI_GetYAxisTicks(&tick_count);

    for (uint8_t i = 0U; i < tick_count; i++)
    {
        int16_t y = HMI_MapY(ticks[i].value, min_value, max_value);
        uint8_t major = (ticks[i].label != NULL) ? 1U : 0U;

        HMI_DrawYAxisTickMark(y, major);

        if (major != 0U)
        {
            HMI_DrawYMajorGridLine(y);
            HMI_PrintYAxisLabel(ticks[i].label, y);
        }
    }
}

static void HMI_DrawYGridColumn(uint16_t x, float min_value, float max_value)
{
    uint8_t tick_count = 0U;
    const HmiYAxisTick_t *ticks = HMI_GetYAxisTicks(&tick_count);
    ...
    for (uint8_t i = 0U; i < tick_count; i++)
    {
        ...
        {
            int16_t y = HMI_MapY(ticks[i].value, min_value, max_value);
            HMI_DrawPixel((int16_t)x, y, HMI_DIM);
            HMI_DrawPixel((int16_t)(x + 1U), y, HMI_DIM);
        }
    }
}
...
static void HMI_DrawPlotBase(const char *title)
{
    ...
    ILI9341_printString(6U, HMI_PLOT_LEGEND_Y, "blue=target  red=measured", HMI_DIM, HMI_BG, HMI_TEXT_SCALE);
...
static void HMI_DrawAxisTitles(const char *x_label, const char *y_label)
{
    ILI9341_printString((uint16_t)(HMI_PLOT_X + 2U),
                        (uint16_t)(HMI_PLOT_Y - 12U),
                        y_label,
                        HMI_DIM,
                        HMI_BG,
                        HMI_TEXT_SCALE);
    ILI9341_printString((uint16_t)(HMI_PLOT_X + 92U),
                        (uint16_t)(HMI_PLOT_Y + HMI_PLOT_H + 18U),
                        x_label,
                        HMI_DIM,
                        HMI_BG,
                        HMI_TEXT_SCALE);
...
static void HMI_ClearTimePlotGraphArea(float min_value, float max_value)
{
    ...
    HMI_DrawYAxisScale(min_value, max_value);
    HMI_DrawTimeTicks();
    hmi_plot_x = 0U;
}
...
static void HMI_DrawTimePlotFrame(void)
{
    ...
    if (desired_state == STATE_VELOCITY_CONTROL)
    {
        ...
        min_value = -CONTROL_DEFAULT_POS_RANGE_RPM;
        HMI_DrawPlotBase("VELOCITY LIVE TIME PLOT");
        HMI_DrawAxisTitles("Time (s)", "Velocity (RPM)");
    }
    else
    {
        ...
        HMI_DrawAxisTitles("Time (s)", "Position (deg)");
    }
...
static void HMI_UpdateTimePlot(void)
{
    ...
    HMI_DrawTimeTickAtOffset(xoff);
    HMI_DrawYGridColumn(x, min_value, max_value);
...
static void HMI_DrawFaultResetButton(void)
{
    uint16_t fill = HMI_SELECTED;
    uint16_t fg = HMI_FG;
    uint16_t border = COLOR_RED;

    ILI9341_fillRect(FAULT_SCREEN_RESET_X, FAULT_SCREEN_RESET_Y,
                     FAULT_SCREEN_RESET_W, FAULT_SCREEN_RESET_H, fill);
    HMI_DrawRectBorder(FAULT_SCREEN_RESET_X, FAULT_SCREEN_RESET_Y,
                       FAULT_SCREEN_RESET_W, FAULT_SCREEN_RESET_H, border, 2U);

    ILI9341_printString((uint16_t)(FAULT_SCREEN_RESET_X + 52U),
                        (uint16_t)(FAULT_SCREEN_RESET_Y + 10U),
                        "RESET", fg, fill, HMI_TEXT_SCALE);
}

static void HMI_DrawFaultScreen(void)
{
    ILI9341_setRotation(1U);
    ILI9341_fillRect(0U, 0U, 320U, 240U, HMI_BG);

    ILI9341_printString(8U, FAULT_SCREEN_TITLE_Y,
                        "FAULT SCREEN", HMI_FG, HMI_BG, HMI_TEXT_SCALE);
    ILI9341_printString(8U, 32U,
                        "Latched faults:", HMI_DIM, HMI_BG, HMI_TEXT_SCALE);

    uint16_t y = FAULT_SCREEN_LIST_Y;
    if (Control_HasFault(CONTROL_FAULT_OVERCURRENT) != 0U)
    {
        ILI9341_printString(FAULT_SCREEN_LIST_X, y,
                            "* OVER CURRENT", COLOR_RED, HMI_BG, HMI_TEXT_SCALE);
        y = (uint16_t)(y + 18U);
    }

    if (Control_HasFault(CONTROL_FAULT_NO_MOTION) != 0U)
    {
        ILI9341_printString(FAULT_SCREEN_LIST_X, y,
                            "* NO MOTOR MOTION", COLOR_RED, HMI_BG, HMI_TEXT_SCALE);
        y = (uint16_t)(y + 18U);
    }
    ...
    {
        ILI9341_printString(FAULT_SCREEN_LIST_X, y,
                            "No latched faults", HMI_FG, HMI_BG, HMI_TEXT_SCALE);
        y = (uint16_t)(y + 18U);
    }
    ...
    {
        ILI9341_printString(FAULT_SCREEN_LIST_X, y,
                            "OC input still LOW", COLOR_RED, HMI_BG, HMI_TEXT_SCALE);
        y = (uint16_t)(y + 18U);
    }

    if (hmi_fault_reset_failed != 0U)
    {
        ILI9341_printString(FAULT_SCREEN_LIST_X, y,
                            "Reset blocked", COLOR_RED, HMI_BG, HMI_TEXT_SCALE);
        y = (uint16_t)(y + 18U);
    }

    ILI9341_printString(FAULT_SCREEN_LIST_X, 172U,
                        "Press RESET after fixing fault", HMI_DIM, HMI_BG, HMI_TEXT_SCALE);

    HMI_DrawFaultResetButton();
}

static void HMI_OpenFaultScreen(void)
{
    hmi_page = HMI_PAGE_FAULT;
    hmi_editing = 0U;
    hmi_tracking_lock = 0U;
    hmi_fault_reset_failed = 0U;
    HMI_ResetEncoderAccumulator();
    HMI_DrawFaultScreen();
}
...
static void HMI_FaultResetSelected(void)
{
    if (Control_ClearFaults() != 0U)
    {
        hmi_fault_reset_failed = 0U;
        HMI_RedrawFullDiagramPage();
    }
    ...
    {
        hmi_fault_reset_failed = 1U;
        HMI_DrawFaultScreen();
    }
}

static void HMI_FaultTask(void)
{
    (void)HMI_ConsumeEncoderSteps();

    if (Button_WasPressed() != 0U)
    {
        HMI_FaultResetSelected();
    }
}

static uint8_t HMI_ServiceFaultState(void)
{
    if ((Control_HasAnyFault() != 0U) && (hmi_page == HMI_PAGE_PLOT))
    {
        hmi_tracking_lock = 0U;
        HMI_RedrawFullDiagramPage();
        return 1U;
...
static const char *HMI_GetPresetFieldName(uint8_t field)
{
    switch (field)
    {
        ...
        case PRESET_ITEM_SAVE_CUR: return "SAVE CURRENT";
...
void UI_Init(void)
{
    ...
    hmi_tracking_lock = 0U;
    hmi_fault_flash_on = 0U;
    hmi_fault_present_last = Control_HasAnyFault();
    hmi_last_fault_flash_ms = HAL_GetTick();
    hmi_fault_reset_failed = 0U;
    hmi_last_value_refresh_ms = HAL_GetTick();
    hmi_last_plot_refresh_ms = HAL_GetTick();
...
void UI_Task(void)
{
    if (HMI_ServiceFaultState() != 0U)
    {
        return;
    }

    if (hmi_page == HMI_PAGE_FAULT)
    {
        HMI_FaultTask();
        return;
    }
    ...
    if ((now - hmi_last_value_refresh_ms) >= HMI_VALUE_REFRESH_MS)
    {
        ...
        HMI_RenderDiagramItemsIfChanged();
    }
}
...
void UI_Draw(void)
{
    ...
    else if (hmi_page == HMI_PAGE_FAULT)
    {
        HMI_DrawFaultScreen();
    }
    else
