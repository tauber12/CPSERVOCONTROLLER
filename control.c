...
volatile uint32_t control_fault_flags = 0UL;
...
static float control_no_motion_timer_s = 0.0f;
...
static void Control_ResetLoopState(void)
{
    PI_Reset(&ctx_pos);
    PI_Reset(&ctx_vel);
    control_no_motion_timer_s = 0.0f;
...
{
    return ((control_fault_flags & fault_mask) != 0UL) ? 1U : 0U;
}

uint8_t Control_HasAnyFault(void)
{
    return (control_fault_flags != 0UL) ? 1U : 0U;
}
...
const char *Control_GetFaultName(uint32_t fault_mask)
{
    switch (fault_mask)
    {
        case CONTROL_FAULT_OVERCURRENT:
            return "OVER CURRENT";
        case CONTROL_FAULT_NO_MOTION:
            return "NO MOTOR MOTION";
        default:
            return "UNKNOWN";
    }
}
...
void Control_RaiseFault(uint32_t fault_mask)
{
    ...
    control_fault_flags |= fault_mask;
    current_state = STATE_FAULT;
    tracking_toggle_request = 0U;

    /* Do not modify TIM1/PWM hardware here. The existing STATE_FAULT path in
     * TIM5_IRQHandler commands zero motor velocity using the normal control
     * path, then the UI exits the plot page.
     */
...
{
    /* Do not clear the latch if the hardware fault input is still active. */
    ...
    OverCurrent_Rearm();
    if (OverCurrent_IsLatched() != 0U)
    {
        return 0U;
    }

    control_fault_flags = 0UL;
...
void Control_EnterTracking(void)
{
    ...
    if (OverCurrent_FaultPending() != 0U)
    {
        Control_RaiseFault(CONTROL_FAULT_OVERCURRENT);
        return;
    }
...
{
    /*
     * Historical function name kept so main.c does not need to change.
     * Presets now initialize from CONTROL_PRESET1_* and CONTROL_PRESET2_*
     * defines in control.h instead of copying the current active gains into
     * both preset slots.
     */
#if (CONTROL_PRESET_COUNT > 0U)
    control_presets[0U].pos_kp = CONTROL_PRESET1_POS_KP;
    control_presets[0U].pos_ki = CONTROL_PRESET1_POS_KI;
    control_presets[0U].vel_kp = CONTROL_PRESET1_VEL_KP;
    control_presets[0U].vel_ki = CONTROL_PRESET1_VEL_KI;
#endif

#if (CONTROL_PRESET_COUNT > 1U)
    control_presets[1U].pos_kp = CONTROL_PRESET2_POS_KP;
    control_presets[1U].pos_ki = CONTROL_PRESET2_POS_KI;
    control_presets[1U].vel_kp = CONTROL_PRESET2_VEL_KP;
    control_presets[1U].vel_ki = CONTROL_PRESET2_VEL_KI;
#endif
}
...
static uint8_t Control_CheckNoMotionFault(float pwm_duty, float measured_velocity)
{
    if ((Control_IsTrackingEnabled() == 0U) || (Control_HasAnyFault() != 0U))
    {
        control_no_motion_timer_s = 0.0f;
        return 0U;
    }

    float dt_s = ctx_vel.dt;
    if (dt_s <= 0.0f)
    {
        dt_s = (control_velocity_loop_hz > 0.0f) ?
               (1.0f / control_velocity_loop_hz) : 0.001f;
    }

    uint8_t pwm_saturated =
        (Control_AbsF(pwm_duty) >= CONTROL_NO_MOTION_PWM_THRESHOLD) ? 1U : 0U;
    uint8_t velocity_near_zero =
        (Control_AbsF(measured_velocity) <= CONTROL_NO_MOTION_VELOCITY_RPM_MAX) ? 1U : 0U;

    if ((pwm_saturated != 0U) && (velocity_near_zero != 0U))
    {
        if (control_no_motion_timer_s < CONTROL_NO_MOTION_FAULT_TIME_S)
        {
            control_no_motion_timer_s += dt_s;
        }

        if (control_no_motion_timer_s >= CONTROL_NO_MOTION_FAULT_TIME_S)
        {
            Control_RaiseFault(CONTROL_FAULT_NO_MOTION);
            return 1U;
        }
    }
    else
    {
        float decay_s = dt_s * CONTROL_NO_MOTION_DECAY_MULTIPLIER;
        if (control_no_motion_timer_s > decay_s)
        {
            control_no_motion_timer_s -= decay_s;
        }
        else
        {
            control_no_motion_timer_s = 0.0f;
...
void TIM5_IRQHandler(void)
{
    ...
    if ((TIM5->SR & TIM_SR_UIF) != 0U)
    {
        ...
        if ((Control_IsTrackingEnabled() != 0U) && (OverCurrent_FaultPending() != 0U))
        {
            Control_RaiseFault(CONTROL_FAULT_OVERCURRENT);
        }
        ...
        if (Control_CheckNoMotionFault(pwm_duty, measured_velocity) != 0U)
        {
            update_Motor_Velocity(0.0f);
            return;
        }
